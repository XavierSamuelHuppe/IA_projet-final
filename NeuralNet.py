...
class NeuralNet:  # nom de la class à changer
    ...
    def cross_validation(self, train, train_labels, k_folds):

        k_list = np.array_split(range(len(train)), k_folds)
        test_part_of_fold = 0
        train_list = []
        test_list = []

        # Création de la liste contenant les parties test de chaque fold
        for i in k_list:
            test_list.append(k_list[test_part_of_fold])
            test_part_of_fold += 1

        test_part_of_fold = 0

        print(len(k_list))
        # Création de la liste contenant les parties train de chaque fold
        for i in k_list:

            print(i)
            test_list_temp = test_list
            returnvalue = test_list_temp.pop(test_part_of_fold)
            #print(returnvalue)
            test_part_of_fold += 1

            """
            flat_list = []
            for sublist in test_list_temp:
                for item in sublist:
                    flat_list.append(item)

            train_list.append(flat_list)
